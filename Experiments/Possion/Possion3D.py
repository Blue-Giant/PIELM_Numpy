...
class GELM(object):
    ...
    def assemble_matrix2inner(self, XY_inner=None, fside=None, if_lambda2fside=True):
        ...
        A = self.ELM_Bases.assemble_matrix2Laplace_3D(XYZ_input=XY_inner)
    ...
    def assemble_matrix2boundary(self, XY_boundary=None, gside=None, if_lambda2gside=True):
        ...
        Z = np.reshape(XY_boundary[:, 2], newshape=[-1, 1])
    ...
    def test_model(self, points=None):
        ELM_Bases = self.ELM_Bases.assemble_matrix2interior_3D(XYZ_input=points)
        return ELM_Bases
